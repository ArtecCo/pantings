...
function AdminLogo({ compact = false }) {
  return <div className={`admin-logo${compact ? ' compact' : ''}`} aria-label="Swarna Tanjore">
    <div className="admin-logo-mark">S</div>
    <div className="admin-logo-copy"><div className="admin-logo-name">Swarna Tanjore</div><div className="admin-logo-subtitle">DIVINE HERITAGE ART</div></div>
  </div>
}
function AdminLayout({ children }) {
  const [menuOpen, setMenuOpen] = useState(false); const closeMenu = () => setMenuOpen(false)
  const handleLogout = async () => { try { await fetch(apiUrl('admin/logout.php'), { method:'POST', credentials:'include' }) } catch(error) { console.error('Logout failed:', error) } window.location.replace('/login') }
  return <div className="admin-app"><header className="admin-topbar"><div className="admin-brand"><button className="mobile-menu-button" onClick={() => setMenuOpen(true)} aria-label="Open menu"><span/><span/><span/></button><AdminLogo/></div><div className="admin-topbar-right"><span className="admin-role">SUPER ADMIN</span><div className="admin-profile"><div className="profile-avatar">A</div><div><strong>Administrator</strong><small>Administrator</small></div></div><button className="logout-button" onClick={handleLogout}>Logout</button></div></header><div className="admin-body">{menuOpen&&<div className="sidebar-backdrop" onClick={closeMenu}/>}<aside className={`admin-sidebar ${menuOpen?'mobile-open':''}`}><button className="mobile-sidebar-close" onClick={closeMenu} aria-label="Close menu">×</button><div className="admin-sidebar-logo"><AdminLogo compact/></div><div className="sidebar-heading">ATELIER MANAGEMENT</div><nav>{menuItems.map(item=><NavLink key={item.path} to={item.path} end={item.path==='/' } onClick={closeMenu} className={({isActive})=>`admin-nav-link ${isActive?'active':''}`}><span className="nav-icon">{item.icon}</span><span>{item.label}</span></NavLink>)}</nav><div className="sidebar-divider"/><div className="sidebar-heading">ADMINISTRATION</div><nav>{managementItems.map(item=><NavLink key={item.path} to={item.path} onClick={closeMenu} className={({isActive})=>`admin-nav-link ${isActive?'active':''}`}><span className="nav-icon">{item.icon}</span><span>{item.label}</span></NavLink>)}</nav><div className="sidebar-footer"><span>Built by Archith</span></div></aside><main className="admin-content">{children}</main></div></div>
}
...
function ProtectedRoute({children}){const[state,setState]=useState('checking');useEffect(()=>{let mounted=true;fetch(apiUrl('auth/admin-session.php'),{method:'GET',credentials:'include'}).then(async response=>{const data=await response.json().catch(()=>({}));if(!mounted)return;setState(response.ok&&data.success===true&&data.authenticated===true&&data.two_factor_verified===true?'authenticated':'unauthenticated')}).catch(error=>{console.error('Administrator session check failed:',error);if(mounted)setState('unauthenticated')});return()=>{mounted=false}},[]);if(state==='checking')return <div className="admin-app"><div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',color:'#5b1217',fontFamily:'Cormorant Garamond, serif',fontSize:'22px'}}>Checking administrator access…</div></div>;return state==='authenticated'?children:<Navigate to="/login" replace/>}
