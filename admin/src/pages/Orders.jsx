...
export default function Orders() {
  ...
  return <div className="orders-page">
    ...
    <div className="orders-toolbar"><div className="orders-search"><span>⌕</span><input type="text" placeholder="Search order number, customer or phone..." value={search} onChange={event => setSearch(event.target.value)} /></div></div>
    ...
    {loading && <div className="painting-list-message"><span>✦</span>Loading orders...</div>}
    {!loading && error && <div className="painting-list-message painting-list-error">{error}</div>}
    {!loading && !error && filteredOrders.length === 0 && <div className="painting-list-empty"><div className="empty-symbol">◇</div><h2>{orders.length === 0 ? 'No orders have been placed yet' : 'No orders found'}</h2><p>{orders.length === 0 ? 'Customer orders will appear here once they are placed.' : 'Try changing your search or status filter.'}</p></div>}
    {!loading && !error && filteredOrders.length > 0 && <div className="orders-table-card heritage-card"><div className="orders-table-wrap"><table className="orders-table"><thead><tr><th>ORDER</th><th>CUSTOMER</th><th>DATE</th><th>TOTAL</th><th>STATUS</th><th></th></tr></thead><tbody>{filteredOrders.map(order => <tr key={order.id}><td><strong className="order-number">#{order.order_number}</strong></td><td><div className="order-customer"><strong>{order.shipping_name || 'Customer'}</strong><span>{order.shipping_phone || '—'}</span>{order.shipping_city && <small>{order.shipping_city}{order.shipping_state ? `, ${order.shipping_state}` : ''}</small>}</div></td><td><span className="order-date">{formatDate(order.created_at)}</span></td><td><strong className="order-total">₹{formatAmount(order.total_amount)}</strong></td><td><span className={`order-status-badge ${statusClasses[order.status] || ''}`}>{statusLabels[order.status] || order.status}</span></td><td><button type="button" className="order-view-button" onClick={() => navigate(`/orders/${order.id}`)}>View</button></td></tr>)}</tbody></table></div><div className="orders-result-count">Showing {filteredOrders.length} of {orders.length} orders</div></div>}
  </div>
}
