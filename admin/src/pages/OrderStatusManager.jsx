...
export default function OrderStatusManager({ order, onUpdated }) {
  ...
  const [pricing, setPricing] = useState({
    base_amount: order?.base_amount ?? order?.subtotal ?? '',
    customization_amount: order?.customization_amount ?? '0',
    delivery_amount: order?.delivery_amount ?? order?.shipping_amount ?? '0',
