  ...
  const loadData = async () => {
    ...
    try {
      const [
        ...
        fetch(`${API}/paintings/get.php?id=${id}`, {
          credentials: 'include'
        }),
      ...
      if (!paintingData.success) {
        throw new Error(
          paintingData.message || 'Unable to load painting'
        )
      }
      ...
      if (!sizesData.success) {
        throw new Error(
          sizesData.message || 'Unable to load sizes'
        )
      }
      ...
      setForm({
        name: painting.name || '',
        category: painting.category_name || '',
        description: painting.description || '',
        price: painting.price || '',
        width: painting.width || '',
        height: painting.height || '',
        frame: painting.frame || '',
        goldDetails: painting.gold_details || '',
        status: Number(painting.is_active) === 1
          ? 'available'
          : 'unavailable'
      })

      const matchingSize = (sizesData.sizes || []).find(
        size =>
          Number(size.width) === Number(painting.width) &&
          Number(size.height) === Number(painting.height)
      )

      if (matchingSize) {
        setSelectedSize(String(matchingSize.id))
      }
      ...
      toast.error(
        error.message || 'Unable to load painting'
      )
  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.message || 'Unable to update painting'
        )
      }
      ...
      toast.error(
        error.message || 'Unable to update painting'
      )
    ...
    <div className="painting-form-page">
      ...
      <div className="page-header">
        ...
        <div>
          ...
          <p>
            Update the artwork details in the ARAmane Arts collection.
          </p>
        </div>
      ...
      >
        ...
        <section className="heritage-card form-section">
          ...
          <div className="form-section-heading">
            ...
            <p>
              Image management will be connected next.
            </p>
          ...
          <div className="image-upload-area">
            ...
            <p>
              Existing image upload controls will be connected
              after the artwork editing workflow is complete.
            </p>
