...
export default function TrackOrders() {
  ...
  if (!authenticated && !error) {
    return (
      <div
        className="ara-collection-state"
        style={{ textAlign: 'center' }}
      >
        <strong>Sign in to track your orders</strong>
        <p>
          Your orders are securely linked to your account. Sign in to view
          order status, delivery details and tracking information.
        </p>
        <Link to="/login" className="ara-btn ara-btn-primary">
          Sign In
        </Link>
      </div>
    )
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {orders.length === 0 ? (
        <div className="ara-collection-state">
          ...
          <p>
            Once you place an order, its status and delivery details will
            appear here.
          </p>
          <Link
            to="/paintings"
        ...
        <div className="space-y-6">
          {orders.map((order) => (
            ...
            >
              ...
              {(order.tracking_courier || order.tracking_id) && (
