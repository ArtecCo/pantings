...
foreach (['base amount' => $baseAmount, 'customization amount' => $customizationAmount, 'delivery amount' => $deliveryAmount, 'discount amount' => $discountAmount] as $label => $value) {
    if ($value < 0) adminJsonResponse(['success' => false, 'message' => "Invalid {$label}"], 422);
}
...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, order_number, status FROM orders WHERE id = ? FOR UPDATE');
