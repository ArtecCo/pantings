...
try {
    // Make sure the painting exists and is active.
    $stmt = $pdo->prepare(
        'SELECT id, is_active
         FROM paintings
         WHERE id = ?
    ...
    if (!$painting || !(int)$painting['is_active']) {
        jsonResponse([
            'success' => false,
            'message' => 'Painting is unavailable'
        ], 404);
    }
    ...
    // Check whether this painting is already in this user's cart.
    $stmt = $pdo->prepare(
        'SELECT id, quantity
         FROM cart_items
         WHERE cart_id = ?
    ...
    $stmt->execute([$cartId, $paintingId]);
    $existing = $stmt->fetch();
    ...
    if ($existing) {
        ...
        // Add the painting to the user's cart.
        $stmt = $pdo->prepare(
            'INSERT INTO cart_items
                (cart_id, painting_id, quantity)
             VALUES
                (?, ?, ?)'
        );
        $stmt->execute([
    ...
    if ((string)$e->getCode() === '23000') {
        jsonResponse([
            'success' => false,
            'message' => 'This painting is already in your cart. Please try again.'
        ], 409);
    }
    ...
    jsonResponse([
        'success' => false,
        'message' => 'Unable to add painting to cart'
    ], 500);
