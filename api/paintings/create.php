...
$sizeOptions = is_array($data['size_options'] ?? null) ? $data['size_options'] : [];
...
foreach ($sizeOptions as $option) {
    ...
    $cleanSizes[] = ['name'=>$optionName,'width'=>(float)$optionWidth,'height'=>(float)$optionHeight,'unit'=>trim((string)($option['unit'] ?? 'in')) ?: 'in','price'=>(float)$optionPrice,'is_standard'=>!empty($option['is_standard'])];
}
if ($cleanSizes) {
    $standardIndex = 0;
    foreach ($cleanSizes as $i => $option) if ($option['is_standard']) { $standardIndex = $i; break; }
    foreach ($cleanSizes as $i => &$option) $option['is_standard'] = $i === $standardIndex;
    unset($option);
    $price = $cleanSizes[$standardIndex]['price']; $width = $cleanSizes[$standardIndex]['width']; $height = $cleanSizes[$standardIndex]['height'];
}

try {
    $slug = strtolower($name); $slug = preg_replace('/[^a-z0-9]+/', '-', $slug); $slug = trim((string)$slug, '-'); if ($slug === '') $slug = 'painting';
    $baseSlug = $slug; $counter = 1; $slugCheck = $pdo->prepare('SELECT id FROM paintings WHERE slug = ? LIMIT 1');
    while (true) { $slugCheck->execute([$slug]); if (!$slugCheck->fetch()) break; $counter++; $slug = $baseSlug . '-' . $counter; }
    $categoryStmt = $pdo->prepare('SELECT id FROM categories WHERE name = ? AND is_active = 1 LIMIT 1'); $categoryStmt->execute([$categoryName]); $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    if (!$category) adminJsonResponse(['success' => false, 'message' => 'Selected category does not exist'], 400);
    $adminId = (int)$_SESSION['admin_user_id']; $isActive = $status === 'available' ? 1 : 0;
...
} catch (Throwable $e) { error_log('Painting create error: ' . $e->getMessage()); adminJsonResponse(['success' => false, 'message' => 'Unable to create painting'], 500); }
