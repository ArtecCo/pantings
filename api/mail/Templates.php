...
function finalPriceCustomerEmail(string $orderNumber, float $baseAmount, float $customizationAmount, float $deliveryAmount, float $discountAmount, float $finalAmount, string $customerName, string $paymentLink = '', string $notes = ''): array {
    $body='<p style="font-size:14px;line-height:1.7;">Dear '.htmlspecialchars($customerName,ENT_QUOTES,'UTF-8').',</p><p style="font-size:14px;line-height:1.7;">Your final quotation for ARAmane Arts order <strong>'.htmlspecialchars($orderNumber,ENT_QUOTES,'UTF-8').'</strong> is now available.</p>'
        . '<div style="margin:24px 0;padding:18px;background:#f4ebe1;border-left:3px solid #d4af37;font-size:13px;line-height:1.8;">'
        . '<div style="color:#8a7770;font-size:10px;letter-spacing:1px;">FINAL QUOTATION</div>'
        . '<div style="margin-top:10px;">Base price: <strong>₹'.number_format($baseAmount,2,'.',',').'</strong></div>'
        . '<div>Customization: <strong>₹'.number_format($customizationAmount,2,'.',',').'</strong></div>'
