...
function orderCreatedCustomerEmail(string $orderNumber, float $totalAmount, string $customerName): array {
    $body='<p style="font-size:14px;line-height:1.7;">Dear '.htmlspecialchars($customerName,ENT_QUOTES,'UTF-8').',</p>'
        .'<p style="font-size:14px;line-height:1.7;">Thank you for placing your order request with ARAmane Arts. Your request has been received and will be reviewed by the artist.</p>'
        .'<div style="margin:24px 0;padding:18px;background:#f4ebe1;border-left:3px solid #d4af37;"><div style="color:#8a7770;font-size:10px;letter-spacing:1px;">ORDER</div><div style="margin-top:6px;color:#5b1217;font-family:Georgia,serif;font-size:22px;">'.htmlspecialchars($orderNumber,ENT_QUOTES,'UTF-8').'</div><div style="margin-top:10px;color:#2c1810;font-size:13px;">Requested amount: <strong>₹'.number_format($totalAmount,2,'.',',').'</strong></div></div>'
        .'<p style="font-size:13px;line-height:1.7;color:#6e5b53;">The artist will get in touch if any clarification is required. Payment options and expected delivery details will be provided after your order is accepted.</p>';
...
function orderStatusCustomerEmail(string $orderNumber, string $status, string $customerName, string $notes = ''): array {
    ...
    $body='<p style="font-size:14px;line-height:1.7;">Dear '.htmlspecialchars($customerName,ENT_QUOTES,'UTF-8').',</p>'
        .'<p style="font-size:14px;line-height:1.7;">There is an update to your ARAmane Arts order.</p>'
...
function orderCreatedAdminEmail(array $order): array {
    ...
    $body='<p style="font-size:14px;line-height:1.7;">A new order request has been received and is awaiting artist review.</p>'
...
function adminOtpEmail(string $otp, string $adminEmail): array {
    $body='<p style="font-size:14px;line-height:1.7;">A sign-in verification code was requested for your ARAmane Arts administrator account.</p>'
        .'<div style="margin:26px 0;text-align:center;padding:22px;background:#f4ebe1;border:1px solid #d4af37;"><div style="color:#8a7770;font-size:10px;letter-spacing:1.5px;">VERIFICATION CODE</div><div style="margin-top:10px;color:#5b1217;font-family:Georgia,serif;font-size:36px;letter-spacing:8px;">'.htmlspecialchars($otp,ENT_QUOTES,'UTF-8').'</div><div style="margin-top:9px;color:#6e5b53;font-size:11px;">Valid for 10 minutes</div></div>'
        .'<p style="font-size:12px;line-height:1.7;color:#6e5b53;">If you did not request this code, secure your administrator account and review your access settings.</p>';
